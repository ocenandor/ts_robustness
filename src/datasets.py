...
class FordDataset(Dataset):
    def __init__(self, path, seq_length=500, step=25) -> None:
        super().__init__()
        ...
        self.labels = [self.labels[i] * np.ones(self.get_num_subarrays(self.data[i],
                                                                       ...
                                                                       step=step)) for i in range(len(self.data))]
        self.data = [self.generate_subarrays(self.data[i], window_size=seq_length, step=step) for i in range(len(self.data))]
